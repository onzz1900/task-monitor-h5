...
用户 / 角色 / 菜单按官方若依字段落在 MySQL：`sys_user`、`sys_role`、`sys_menu`、`sys_dept`、`sys_user_role`、`sys_role_menu`，对照 [RuoYi-Vue](https://github.com/yangzongzhuan/RuoYi-Vue) `sql/ry_20260417.sql`。登录仍是 Better Auth。非超管侧栏按 `sys_role_menu` 少显示；未登录 `/dashboard/*` 回 Login v1；停用账号下一请求失效。
