...
- **主后端（系统记录源）**：Python FastAPI（`../backend/`），SQLite 文件存储（`backend/data/tms.db`，首次启动自动建表 + 写入演示数据），SQLAlchemy 模型可平移到 Postgres
- **认证**：[fastapi-users](https://fastapi-users.github.io/fastapi-users/)（密码哈希与 JWT 签发由库处理）；RBAC 用角色 / 权限 / 菜单表实现，接口级校验
- **前端**：Next.js App Router + React + TypeScript + Tailwind CSS + shadcn/ui
- **BFF**：Next.js Route Handlers 仅作**薄代理**（`/api/* → FastAPI`，把 httpOnly Cookie 中的 JWT 换成 Bearer 头），不落任何数据、不做业务逻辑
- **视觉**：默认**纸票版**皮肤，顶栏可切换**控制台版**深色皮肤；`variants/` 下的静态原型保持原样作为存档

## 启动（两个进程）

```bash
# 1) 主 API（FastAPI，端口 8000）
cd backend
python3 -m venv .venv
...
打开 <http://localhost:3000> 即到登录页。前端通过 `TMS_API_URL` 环境变量指向主 API（默认 `http://localhost:8000`）。
...
菜单按权限过滤：只读账号看不到「用户管理」「角色权限」，任务页也没有登记 / 运行按钮；FastAPI 接口同样校验（403）。

## 数据模型（backend/app/models.py）

SQLite 表结构只用可移植类型，可平移到 Postgres。fastapi-users 提供 `users` 基础字段（email / hashed_password / is_active…）：
...
| `users` | `…fastapi-users 基础字段 + name, role_id` | 用户挂角色 |
...
| | `schedule_kind, cron_expr, interval_minutes, timezone, next_run_at` | 调度：cron（croniter 解析）或固定间隔，固定 `Asia/Shanghai`；`next_run_at` 即「下次流转」 |
| | `callback_url, callback_timeout_ms, callback_retries, callback_secret_ref` | 可选 POST 回调；超时默认 10000ms、重试默认 3；只存 **secretRef** 引用名，密钥本体在服务端密管，永不下发 |
...
| 方法 | 路径 | 权限 |
| --- | --- | --- |
| POST | `/auth/jwt/login` | fastapi-users 托管（表单 `username` / `password`） |
...
前端一律请求同源 `/api/*`，由 BFF 代理转发；`/api/auth/login`、`/api/auth/logout` 是仅有的两个非透传路由（Cookie 存取）。
