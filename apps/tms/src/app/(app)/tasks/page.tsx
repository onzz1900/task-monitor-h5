...
export default function TasksPage() {
  ...
  useEffect(() => {
    Promise.all([api<Task[]>("/api/tasks"), api<Meta>("/api/meta")])
      .then(([t, m]) => {
        setTasks(t);
        setMeta(m);
      })
  ...
  const stats = useMemo(() => {
    const by = (s: string) => tasks?.filter((t) => t.status === s).length ?? 0;
    return [
  ...
  if (!tasks || !meta) return <p className="text-muted-foreground">载入中…</p>;
