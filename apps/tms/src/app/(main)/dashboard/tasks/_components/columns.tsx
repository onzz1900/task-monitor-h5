...
function TaskRowActions({ task }: { task: Task }) {
  ...
  const numericId = task.numericId;
    ...
    <div className="text-right">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-40">
          <DropdownMenuItem
            onSelect={() => {
              if (numericId) router.push(`/dashboard/tasks/${numericId}/edit`);
            }}
          >
            Edit
          </DropdownMenuItem>
          ...
          <DropdownMenuItem
            onSelect={() => {
              if (!numericId) return;
              void api(`/api/tasks/${numericId}/run`, { method: "POST" }).then(() => {
                router.refresh();
              });
            }}
          >
...
export const columns: ColumnDef<DataTableFeatures, Task>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const href = task.numericId ? `/dashboard/tasks/${task.numericId}` : undefined;
      return href ? (
        <Link prefetch={false} href={href} className="inline-block w-20 font-mono text-muted-foreground text-sm hover:underline">
          {row.getValue("id")}
        </Link>
      ) : (
        <div className="w-20 font-mono text-muted-foreground text-sm">{row.getValue("id")}</div>
      );
    },
    enableSorting: false,
    enableHiding: false,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const task = row.original as Task;
      return <TaskRowActions task={task} />;
    },
  },
];
